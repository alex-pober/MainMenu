...
export function MenuDetails({
  ...
  const isCurrentlyAvailable = (menu: Menu) => {
    ...
    return (
      currentTime >= menu.available_start_time! &&
        ...
        activeFilters.every((filter) => {
          switch (filter) {
            ...
            case "new":
              return item.is_new;
            case "limited":
              return item.is_limited_time;
            case "popular":
              return item.is_most_popular;
            case "special":
    ...
    <div>
      ...
      <div
        className={cn(
          ...
          isSidebarOpen &&
            "lg:opacity-50 pointer-events-none transition-opacity duration-300"
        )}
      >
        {/* THIS IS THE MENU CATEGORIES LIST */}
        {categories.map((category) => {
          const filteredItems = filterItems(category.menu_items);
          if (activeFilters.length > 0 && filteredItems.length === 0)
            return null;
        ...
        <div className="h-full flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="p-4 lg:p-6 pb-32 divide-y divide-border">
              {selectedCategory &&
                filterItems(selectedCategory.menu_items).map((item) => (
                  <div
                    key={item.id}
                    ...
                    {item.image_urls?.[0] && (
                      <div className="w-full mb-4">
                        ...
                        <div
                          ...
                          onTouchEnd={(e) =>
                            onTouchEnd(e, true, item.image_urls)
                          }
                        >
                          {/* Blurred Background */}
                          <div
                            className="absolute inset-0 z-0 blur-xl scale-150 bg-center bg-contain bg-no-repeat"
                            style={{
                              backgroundImage: `url(${
                          ...
                          {item.image_urls.length > 1 && (
                            <>
                              <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex gap-1.5 px-2 py-1 bg-black/30 rounded-full z-20">
                        ...
                        {(item.is_spicy ||
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            ...
                            {item.is_vegetarian && (
                              <Badge
                                variant="outline"
                                className="text-xs bg-lime-100 text-lime-800 border-lime-200"
                              >
                                🥚 Vegetarian
                              </Badge>
                            )}
